...
row = 1;
for p = 1:nPatches
    rho = rho_patches(:,p); % one patch, one depth at a time 

    for i = 1:nDepths
        d = depths(i);
        attenuation = exp(-Kd_true .* d);
        spectrum = rho .* E0 .* attenuation;

        RGB(row,1) = sum(SR .* spectrum) * delta_lambda;  % R Riemann sum for integration 
        RGB(row,2) = sum(SG .* spectrum) * delta_lambda;  % G Riemann sum for integration 
        RGB(row,3) = sum(SB .* spectrum) * delta_lambda;  % B Riemann sum for integration 
