...
% Read the CSV file
tiff_file_data = readtable('E:\Colorimetry\Photos\Perlas\Contadora_28_August_2023\Contadora_28_Aug_2023_0to25\wp_selected_color_charts.csv');
...
masks_file = fullfile(savePath, 'wp_all_masks.mat');
if isfile(masks_file)
    load(masks_file, 'wp_all_masks');
else
    wp_all_masks = struct();  % Initialize all_masks if not loaded
end
...
for idx = 1:length(tiff_file_names)
    ...
    neutralPatches = [1 1; 1 2; 1 3; 1 4; 1 5; 1 6];
    neutralTarget = [0.699 0.243 0.186 0.124 0.079 0.0322];
    ...
    if isfield(wp_all_masks, sanitized_name)
        % Use the mask from all_masks
        current_mask = wp_all_masks.(sanitized_name);
    else
        ...
        wp_all_masks.(sanitized_name) = current_mask;
    end
...
% Loop through each color and overlay the corresponding mask
    for i = 1:length(all_colors_mask)
        ...
        visualized_mask = wp_all_masks.(sanitized_name).(patch_color).mask;  % Binary mask
    ...
    mask_img_savePath = fullfile(savePath, 'wp_masks');  % Example directory
    ...
    mask_img_saveFileName = sprintf('wp_mask_overlay_%s.png', linear_tiff);
...
% % title(sprintf('%s Mask Overlay', linear_tiff), 'Interpreter', 'none');
% 
% mask_img_savePath = fullfile(savePath, 'wp_depth_mask_images');  % Example directory
% 
% % Create the directory if it doesn't exist
% if ~exist(mask_img_savePath, 'dir')
%     mkdir(mask_img_savePath);
...
% imwrite(combinedImage, mask_img_saveImagePath);
    ...
    % Save plots 
    plot_save_path = fullfile(savePath, 'wp_linearity_check');
    if ~exist(plot_save_path, 'dir')
    mkdir(plot_save_path); % make folder called linearity check and save plots here 
    end

    lin_fig = figure('Visible', 'off');
    ...
    xlabel('Gray Patch Y value (luminance)')
    ylabel('RGB values')
    legend('Red channel', 'Green channel', 'Blue channel', 'Location', 'northwest')

    plot_file_name = fullfile(plot_save_path, sprintf('camera_sensor_linearity_%d.png', idx));
    saveas(lin_fig, plot_file_name);
    close(lin_fig);

    %% Plotting Y values vs. RGB values
    % Only Display plot, dont save  
...
save_csv_file_single_row = fullfile(savePath, 'wp_rgb_depth_bc.csv');
...
depth_vs_bc = readtable(fullfile(savePath, 'wp_rgb_depth_bc.csv'));
...
% Create the plot
back_fig = figure; % Store figure handle
...
% Save the figure
backscatter_plot_file_name = fullfile(savePath, 'wp_backscatter_vs_distance.png');
saveas(back_fig, backscatter_plot_file_name);
